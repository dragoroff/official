...
@accepts('post')
@json_request
def make_withdrawal(request, acc_id):
    ...
    data = {'from': acc_id, 'amount': amount}
    state = {'event': EVENT_WITHDRAWAL_REQUESTED, 'data': data}
    with transaction.atomic():
        producer.send(kafka_topic, value=state)
        Events.objects.create(entityID=acc_id, event_type=EVENT_WITHDRAWAL_REQUESTED, data=data)
